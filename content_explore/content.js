...
document.addEventListener("DOMContentLoaded", function() {
    ...
    //important for next page details
    Rank=rank;
    Region=region;
...
async function fetchMoviesByPageAndType(page,type,rank,region) {
    ...
    { 
    response = await fetch(`https://bingflix-backend.vercel.app/explore/popular/${type}/${page}`);
    }
    else if (rank=='topRanked' && region==null) //Top-rated US
    {
        console.log('yes')
    response = await fetch(`https://bingflix-backend.vercel.app/explore/top_rated/${type}/${page}`);
    }
    else if (rank=='topRanked' && region == "KR") //Popular Korean
    {
    response = await fetch(`https://bingflix-backend.vercel.app/explore/pop_korea/${page}`);
    }

    else if (rank=='topRanked' && region == "IN") //Popular Indian
    {
    response = await fetch(`https://bingflix-backend.vercel.app/explore/pop_india/${page}`);
    }
...
async function fetchMovieDetails(movieId,type) {
    const response = await fetch(`https://bingflix-backend.vercel.app/explore/content_details/${type}/${movieId}`);
...
async function getYouTubeTrailerUrl(movie,type) {
    const response = await fetch(`https://bingflix-backend.vercel.app/explore/youtube/${type}/${movie.id}`);
    const data = await response.json();
